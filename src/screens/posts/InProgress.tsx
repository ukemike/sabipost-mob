...
const InProgress = () => {
  ...
  const pendingAndQuotedPosts = useMemo(
    () =>
      allPosts?.filter(
        (post: any) => post.status === "pending" || post.status === "quoted"
      ),
    [allPosts]
  );
            ...
            <VStack flex={1} space="lg">
              <VStack space="sm">
                <Text
                  ...
                  textAlign="left"
                >
                  Records found: {pendingAndQuotedPosts?.length}
                </Text>

                <FlatList
                  data={pendingAndQuotedPosts}
